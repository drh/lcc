		      ...
		      if (tp->kids[0]->op == FIELD) {
			      ...
			      if (fieldsize(f) < 8*f->type->size) {
				      ...
				      ||  q->op == CNST+U && q->u.v.u == 0)
				              q = bittree(BAND, x, cnsttree(unsignedtype, ~mask));
				      else if (q->op == CNST+I && (q->u.v.i&fmask) == fmask
				      ||       q->op == CNST+U && (q->u.v.u&fmask) == fmask)
				              q = bittree(BOR, x, cnsttree(unsignedtype, mask));
				      else {
				              listnodes(q, 0, 0);
				              q = bittree(BOR,
				                      bittree(BAND, rvalue(lvalue(x)),
				                              cnsttree(unsignedtype, ~mask)),
				                      bittree(BAND, shtree(LSH, cast(q, unsignedtype),
				                              cnsttree(unsignedtype, fieldright(f))),
				                              cnsttree(unsignedtype, mask)));
				      }
			              r = listnodes(q, 0, 0);
			              op = ASGN + ttob(q->type);
...
static Node visit(Node p, int listed) {
		...
		&& isaddrop(p->kids[0]->op)) {
		        if (p->kids[0]->syms[0]->sclass == REGISTER) {
