...
$Brcc$E:	$Bmain$O $Blibrcc$A
		$(LD) $(LDFLAGS) -o $@ $Bmain$O $Blibrcc$A
...
$Bsparc$O:	$Bsparc.c src/stab.h;	$(CC) $(CFLAGS) -c -Isrc -o $@ $Bsparc.c
...
clean::		testclean
		$(RM) $B*$O
		$(RM) $Bdagcheck.c $Balpha.c $Bmips.c $Bx86.c $Bsparc.c $Bgram.c
		$(RM) $Brcc1$E $Brcc1$E $B1rcc$E $B2rcc$E
		$(RM) $B*.ilk

clobber::	clean
		$(RM) $Brcc$E $Blburg$E $Bcpp$E $Blcc$E $Bbprint$E $B*$A
		$(RM) $B*.pdb
...
C=$Blcc -A -d0.6 -Wo-lccdir=$(BUILDDIR) -Isrc
